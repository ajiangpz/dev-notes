---
title: reasy-ui组件库入口文件解析
date: "2025-07-04"
tags: ["webpack"]
summary: "reasy-ui组件库入口文件解析"
---
...
## install 方法的作用
Vue.use(ReasyUI) 会执行 install 方法，将入口文件引入的组件注册为全局组件

```js
// 完整引入
import Vue from 'vue'
import ReasyUI from 'reasy-ui'
Vue.use(ReasyUI)  // 这里会调用install函数

// 之后可以在任何组件中使用
this.$message.success('操作成功')  // 使用全局方法
<r-button>点击</r-button>         // 使用全局组件
